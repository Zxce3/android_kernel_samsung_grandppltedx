...
#define JTAG_ADDR1_BASE 0x102D0000
#define JTAG_ADDR2_BASE 0x104A0000
#define JTAG_ADDR3_BASE 0x10460000
...
static VOID consys_set_if_pinmux(MTK_WCN_BOOL enable)
{
	...
	UINT8 *consys_if_drv_pinmux_reg_base = NULL;
	...
	if (enable) {
		...
		consys_if_drv_pinmux_reg_base = ioremap_nocache(CONSYS_IF_DRV_PINMUX_REG_BASE, 0x4);
		if (consys_if_drv_pinmux_reg_base)
			CONSYS_REG_WRITE(consys_if_drv_pinmux_reg_base,
					(CONSYS_REG_READ(consys_if_drv_pinmux_reg_base) &
					 CONSYS_IF_DRV_PINMUX_MASK));
		else
			WMT_PLAT_ERR_FUNC("consys_if_drv_pinmux_reg_base(%x) ioremap fail\n",
	...
	if (consys_if_drv_pinmux_reg_base)
		iounmap(consys_if_drv_pinmux_reg_base);
}
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		/*set vcn18 SW mode*/
		KERNEL_upmu_set_reg_value(MT6355_LDO_VCN18_OP_EN, 0x1);
		...
		/*set vcn33 SW mode*/
		KERNEL_upmu_set_reg_value(MT6355_LDO_VCN33_OP_EN, 0x1);
